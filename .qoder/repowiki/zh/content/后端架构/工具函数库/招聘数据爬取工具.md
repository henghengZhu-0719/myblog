...
- 增强了异步处理能力和错误处理机制
- 改进了超时配置和详细的执行结果跟踪
- 更新了爬虫工具的API接口和返回结果格式
- 增加了详细的URL执行状态报告和错误诊断信息
...
**更新** 本版本增强了异步处理能力，改进了错误处理机制，增加了超时配置和详细的执行结果跟踪，更新了API接口和返回结果格式。
...
- 异步处理示例：utils/bill.py、routers/bill.py
...
AsyncExample["异步处理示例<br/>utils/bill.py & routers/bill.py"]
end
subgraph "数据与配置"
...
- **新增** 执行结果跟踪：详细的URL执行状态、错误信息和新增数量报告。

**更新** 增强了执行结果跟踪能力，提供每个URL的详细执行状态和诊断信息。
...
Crawler-->>BG : 返回详细结果列表
BG-->>API : 更新执行状态
Client->>API : GET /api/actions/crawl/result
API-->>Client : 返回执行结果与状态
```
...
- **新增** 详细结果跟踪：返回每个URL的执行状态、消息和新增数量，提供完整的执行诊断信息。
- 主流程：读取 targets.txt → 匹配规则 → 抓取 → 解析 → 去重 → 保存 → 邮件 → 汇总结果。

**更新** 增强了错误处理和结果跟踪，提供详细的URL执行状态报告。

```mermaid
flowchart TD
Start(["开始 run_crawler"]) --> LoadTargets["读取 targets.txt"]
LoadTargets --> LoopURL{"遍历URL"}
LoopURL --> |匹配规则| Fetch["fetch_html 等待选择器<br/>30秒加载超时<br/>10秒元素等待超时"]
LoopURL --> |无匹配| Skip["跳过并记录<br/>状态: skip<br/>消息: 无匹配规则"]
Fetch --> Parse["调用解析器生成Job列表"]
Parse --> Dedup["过滤已存在URL"]
Dedup --> HasNew{"有新增？"}
HasNew --> |是| Save["保存到数据库<br/>逐条提交+回滚"]
HasNew --> |否| Noop["无新增<br/>状态: success<br/>消息: 无新增内容"]
Save --> Mail["发送邮件通知"]
Mail --> Next["继续下一个URL"]
Noop --> Next
Skip --> Next
Next --> LoopURL
LoopURL --> |完成| Result["返回详细结果列表<br/>包含状态、消息、新增数量"]
```
...
- **新增** 执行状态管理：实时跟踪爬取任务的运行状态、结果和完成时间。
...
BG->>Crawler : run_crawler()
Crawler-->>BG : 返回详细结果列表
BG-->>Router : 更新运行状态<br/>running : true/false<br/>results : 执行结果<br/>finished_at : 完成时间
Client->>Router : GET /api/actions/crawl/result
Router-->>Client : 返回执行状态与结果<br/>包含每个URL的详细信息
```
...
### 异步处理示例（bill.py、routers/bill.py）
- **新增** 异步处理模式：展示了如何在FastAPI中使用async/await和线程池处理阻塞操作。
- **新增** 错误处理最佳实践：提供了完整的异常捕获和错误信息返回机制。
- **新增** 并发控制：使用run_in_executor在事件循环中执行同步操作，避免阻塞。

**更新** 增加了异步处理示例，为爬虫系统的异步化改造提供参考。
...
- **新增** 异步处理依赖：async-timeout、anyio等异步处理相关库。
...
AsyncExample["routers/bill.py"] --> AsyncUtils["utils/bill.py"]
AsyncExample -. 参考 .-> Crawl
```
...
- **新增** 超时配置：页面加载超时30秒，元素等待超时10秒，确保系统稳定性。
- **新增** 异步化改造：为未来实现异步爬取提供架构基础。

**更新** 增加了超时配置和异步化改造的性能考虑。
...
- **新增** 执行结果诊断：通过详细的URL执行状态和错误消息进行故障定位。
- **新增** 超时问题排查：检查网络状况和目标网站响应速度。

**更新** 增加了执行结果诊断和超时问题排查指南。
...
## 结论
该招聘数据爬取工具以规则驱动与动态渲染为核心，结合去重与邮件通知机制，形成稳定的抓取-解析-入库-反馈闭环。通过 FastAPI 的后台任务与路由接口，系统具备良好的可运维性与扩展性。**更新** 本版本增强了异步处理能力、改进了错误处理机制、增加了超时配置和详细的执行结果跟踪，为未来的异步化改造奠定了坚实基础。后续可在并发控制、缓存与增量策略、反爬虫对抗等方面进一步优化。
...
- 页面等待超时：直接获取内容，继续解析。
- 保存异常：逐条回滚，记录失败条目。
- **新增** 详细错误报告：提供URL、状态、消息的完整错误信息。
- **新增** 执行结果跟踪：每个URL都有独立的执行状态和诊断信息。
- 建议：引入指数退避与最大重试次数，结合队列与限速。

**更新** 增加了详细错误报告和执行结果跟踪机制。
...
- 新站点适配步骤：
  ...
  6) 开启邮件通知（可选），验证邮件发送。
- **新增** 异步化改造指导：参考bill.py和routers/bill.py的异步处理模式。
- **新增** 错误处理最佳实践：学习详细的错误报告和状态管理机制。
- 参考文件：
  - [crawl.py:249-292](file://blog_backend/utils/crawl.py#L249-L292)
  - [crawl.py:56-246](file://blog_backend/utils/crawl.py#L56-L246)
