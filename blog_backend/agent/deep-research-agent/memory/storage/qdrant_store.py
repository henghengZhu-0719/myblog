...
class QdrantConnectionManager:
    """
    Qdrant数据库连接管理器
    管理与Qdrant数据库的连接，确保线程安全
    """

    _instances = {}  # key: (url, collection_name)
...
class QdrantVectorStore:
    def __init__(
        ...
        **kwargs
        ...
        if not QDRANT_AVAILABLE:
            raise ImportError("Qdrant客户端库未安装，无法初始化Qdrant向量存储")
        ...
        try:
            ...
            logger.warning("QDRANT_HNSW_M环境变量值不是整数，使用默认值32")
            self.hnsw_m = 32
        try:
    ...
    def _ensure_payload_indexes(self):
        ...
        try:
            index_fields = [
                ...
                # Parsing相关索引字段
                ("is_parsing_data", models.PayloadSchemaType.BOOL),
                ("domain", models.PayloadSchemaType.KEYWORD),
                ("rule_type", models.PayloadSchemaType.KEYWORD),
    ...
    def get_collection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 兼容新旧 qdrant-client：vectors_count 在新版已改为 points_count
            vectors_count = (
                getattr(collection_info, "vectors_count", None)
                or getattr(collection_info, "points_count", None)
                or 0
            )
            info = {
