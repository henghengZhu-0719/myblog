...
class WorkingMemory(BaseMemory):
    ...
    def add(self, memory_item: MemoryItem) -> str:
        ...
        # 计算优先级
        priority = self._calculate_importance(memory_item.content)

        
        # 优先级队列，负数优先级，确保高优先级先出
        ...
        for memory in filtered_memories:
            ...
            if query_lower in content_lower:
                ...
                if intersection:
                    ...
                    # 中文字符级别匹配兜底：统计 query 中有多少字符出现在 content 里
                    query_chars = set(query_lower)
                    content_chars = set(content_lower)
                    char_overlap = query_chars.intersection(content_chars)
                    if char_overlap and len(query_chars) > 0:
                        keyword_score = len(char_overlap) / len(query_chars) * 0.5
    ...
    def _calculate_time_decay(self, timestamp: datetime) -> float:
        ...
        # 指数衰减（工作记忆衰减更快）
        decay_factor = 0.9 ** (hours_passed / 6)  # 每6小时衰减
