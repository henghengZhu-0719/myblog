...
流程：
1. 将每篇 tests/data 文档的前半部分发送给 DeepSeek，生成 3~5 个 (dense_query, sparse_query) 对
...
TEST_MD_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data")
REPORT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "reports", "retrieval_reports")
...
class RetrievalTester:
    def __init__(self, top_k: int = 10, use_reranker: bool = True):
        ...
        self._llm = OpenAI(api_key=DEEPSEEK_API_KEY, base_url=DEEPSEEK_BASE_URL)

        embedder = EmbeddingService()
        sparse_encoder = SparseEncoder()
        reranker = RerankerService() if use_reranker else None

        self.vector_store = VectorStore(embedder=embedder, sparse_encoder=sparse_encoder)
...
请根据以上文档内容生成 3~5 个测试查询对。"""
    ...
    def run(self, file_filter: list[str] | None = None) -> list[dict]:
        all_files = sorted(f for f in os.listdir(TEST_MD_DIR) if f.endswith(".md"))
        if file_filter:
    ...
    def _compute_metrics(self, results, all_queries, all_files):
        total = len(results)
        hit_any = sum(1 for r in results if r.source_rank is not None)
        hit_top1 = sum(1 for r in results if r.top1_source_match)

        reciprocal_ranks = [1.0 / r.source_rank if r.source_rank else 0.0 for r in results]
        ...
        for r in results:
            ...
            by_difficulty.setdefault(d, {"total": 0, "hit": 0, "ranks": []})
        ...
        for d in ["easy", "medium", "hard"]:
            if d in by_difficulty:
                ...
                logger.info("  %s: %d/%d = %.1f%%  (平均排名: %.1f)", d, s["hit"], s["total"], hit_rate, avg_rank)
    ...
    def _write_md_report(self, results, all_files, mrr, by_difficulty, total, hit_any):
        ...
        lines = [
            ...
            f"- **测试文件数**: {len(all_files)}",
            f"- **查询总数**: {total}",
            f"- **Top-K**: {self.top_k}",
            f"- **Re-ranker**: {'启用 (qwen3-rerank)' if self.use_reranker else '禁用'}",
            "",
            "## 汇总指标\n",
            "| 指标 | 值 |",
            "|------|-----|",
        ...
        for i, r in enumerate(results, 1):
            ...
            lines.append("")
            lines.append(f"**检索结果 (Top-{min(5, len(r.hits))}):**\n")
...
if __name__ == "__main__":
    ...
    tester = RetrievalTester(top_k=args.top_k, use_reranker=not args.no_rerank)
