#!/usr/bin/env python3
"""
测试脚本：对 tests/data 目录下所有 Markdown 文件进行 chunk 切分，
...
REPORT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", "reports", "chunk_reports")
CHUNK_SIZE = 500
OVERLAP = 75
...
async def process_one(parser: MarkdownSectionParser, filepath: str, filename: str, writer: ReportWriter):
    ...
    chunks = await parser.parse_and_chunk(text=text, source=filename, chunk_size=CHUNK_SIZE, overlap=OVERLAP)
    ...
    if INCLUDE_CHUNK_CONTENT and chunks:
        ...
        for i, chunk in enumerate(chunks[:sample_count], 1):
            ...
            writer.md(f"<details>")
            writer.md(f"<summary><b>Chunk #{i}</b> | tokens: {tokens}"
